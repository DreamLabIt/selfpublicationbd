...
const BANNERS_COLLECTION_TAG = "banners";
const getSingleBannerTag = (id: string) => `banner:${id}`;

function clearBannerCache(bannerId?: string) {
  try {
    revalidatePath(ADMIN_BANNER_PATH);
    revalidatePath(PUBLIC_HOME_PATH);
    revalidateTag(BANNERS_COLLECTION_TAG, "default");
    if (bannerId) {
      revalidateTag(getSingleBannerTag(bannerId), "default");
    }
  } catch (error) {
